...
| 日期 | 主題 | 路徑 | 影響 / 後續 |
|------|------|------|------|
| 2026-09-04 | **Stamp Grid L2 Implementation**：Step 3 新增集點印章區塊（conditional on `stamp_card` / `multipass`）；4 grid sizes（1×5 / 2×5 / 3×5 / 4×5）+ 5 印章圖示 picker；StampGridPreview 共用 component（Plan 2 API：stripHeight/stripWidth + 內部 calculateCellSize）+ 16 個 utils 純函式 unit test；Strip 渲染走透明 padded container；Schema 4 層同步（shared card.ts → backend request.ts → backend db templates.ts → service）；10 個 PNG 從根目錄搬到 `apps/frontend/src/assets/icons/stamps/`；453/453 vitest 全綠 + 13/13 schema-conformance + 17 namespaces verify:i18n 全綠；39 個檔案 commit `e949656` | runs/improvements/feedback/20260904-stamp-grid-feature.md | 給未來 trace 整條 Stamp grid 鏈的人 single entry point；Plan 2 cellSize 公式（嚴格 min-cap）為 dynamic sizing SOP；i18n 沿用 cardEditor namespace（不開新 namespace）；conditional render on cardType 是 stamp 系列卡種擴充的範本 |
...
| 2026-09-01 | **useImageCrop Rectangular Support**：8/30 LogoUploader 重構時 `cropWindowSize` 是 square 假設；background 是第一個非 square variant，UI mask 必須 800×317（aspect 2.52:1）match 1860×738 export；Hook signature 向後相容擴充（square fallback）| runs/improvements/feedback/20260901-use-image-crop-rectangular-support.md | Rule 028 § 11.1 Rectangular Crop Window（NEW）；`useImageCrop` signature 升級為 `cropWindowWidth/Height`；`computeSrcSquareSize` → `computeSrcRegion` |
...
| 2026-09-01 | Rule 028 § 11.1 Rectangular Crop Window（NEW）：`cropWindowWidth/Height` 而非 `cropWindowSize`；`computeSrcRegion` 矩形化；MediaAssetUploader structural cast 取 union cropConfig 欄位；signature 向後相容 | ✅ done (this commit) |
| 2026-09-01 | Rule 028 § 12.1 Stage Width Floor（NEW）：`MIN_STAGE_WIDTH = 200`，不跟 `CROP_WINDOW_*` 走；兩個覆蓋場景（mobile viewport 退化 + jsdom 退化）；magic number 沉澱成 invariant | ✅ done (this commit) |
