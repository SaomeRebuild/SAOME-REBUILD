...
export interface PassCardPreviewProps extends HTMLAttributes<HTMLDivElement> {
  ...
  /**
   * 2026-09-20 multipass card — First multipass tier reward type from
   * CardBuilderEditorPreview (Step 6 `multipassTiers[0].rewardType`).
   * Forwarded to PassCardPreviewBody for the `multipassRewardContent`
   * slot.
   */
  firstMultipassRewardType?: import('@saome/shared/constants/multipass-card').MultipassRewardType | null;
  /**
   * 2026-09-20 multipass card — First multipass tier reward value from
   * CardBuilderEditorPreview (Step 6 `multipassTiers[0].rewardValue`).
   * Interpolated into the amountFormatTWD/ZAR/percentFormat i18n template
   * for the `multipassRewardContent` slot.
   */
